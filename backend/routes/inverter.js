...
const { getPlantStatus, utlFetch } = require("../services/utlApi");
...
router.get("/", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(
        "[Inverter] UTL returned non-JSON (HTTP " + response.status + "): " + text.slice(0, 200),
      );
      return res.status(502).json({ success: false, error: "Upstream returned invalid data." });
    }
    const plantStatus = await getPlantStatus(req.token, req.session);
    ...
    console.log("Final inverter response:", JSON.stringify(data, null, 2));
